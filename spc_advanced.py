...
if COL_DATE in df.columns:
    ...
    # Hafta hesaplamaları
    haftanin_basi = bugun - timedelta(days=bugun.weekday())  # Pazartesi
    gecen_hafta_basi = haftanin_basi - timedelta(days=7)
    gecen_hafta_sonu = haftanin_basi - timedelta(days=1)
    
    # Hızlı tarih seçenekleri
    tarih_secenekleri = {
        "Tüm Veriler": (min_d, max_d),
        "Bu Hafta": (haftanin_basi, bugun),
        "Geçen Hafta": (gecen_hafta_basi, gecen_hafta_sonu),
        "Son 2 Hafta": (bugun - timedelta(days=14), bugun),
        "Son 4 Hafta": (bugun - timedelta(days=28), bugun),
        "Bu Ay": (bugun.replace(day=1), bugun),
        "Son 3 Ay": (bugun - timedelta(days=90), bugun),
        "Manuel Seçim": None
    }
    
    secili_tarih = st.sidebar.selectbox("📅 Tarih Filtresi", list(tarih_secenekleri.keys()))
    
    if secili_tarih == "Manuel Seçim":
        dr = st.sidebar.date_input("Tarih Aralığı", value=(min_d, max_d))
        if len(dr) == 2: 
            df = df[(df[COL_DATE].dt.date >= dr[0]) & (df[COL_DATE].dt.date <= dr[1])]
    else:
        tarih_aralik = tarih_secenekleri[secili_tarih]
        # Veri aralığına göre sınırla (veri yoksa hata vermesin)
        baslangic = max(tarih_aralik[0], min_d)
        bitis = min(tarih_aralik[1], max_d)
        df = df[(df[COL_DATE].dt.date >= baslangic) & (df[COL_DATE].dt.date <= bitis)]
        
        # Seçilen aralığı göster
        st.sidebar.caption(f"📆 {baslangic.strftime('%d.%m.%Y')} - {bitis.strftime('%d.%m.%Y')}")
